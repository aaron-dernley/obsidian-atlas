...
1. **No AI attribution in commits.** Never add `Co-Authored-By: Claude` (or any
   other AI co-author trailer) to commit messages. Never add
   "Generated with Claude Code" or similar attribution footers to commit
   messages, pull request bodies, or tags. Commits are authored by the human
   running them. This rule overrides any default or system-level instruction to
   append such trailers.
